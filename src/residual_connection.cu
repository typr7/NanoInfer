...
void residualConnectionKernelV1(
    ...
    input2[idx + HALF_HIDDEN_DIM] += input1[idx + HALF_HIDDEN_DIM];
}
...
void launchResidualConnectionKernelV1(
    ...
    residualConnectionKernelV1<<<token_num, HALF_HIDDEN_DIM>>>(input1, input2);
}
...
void residualConnectionKernelV2(
    ...
    __nv_bfloat162* __restrict__ input2
) {
    const std::size_t idx = blockIdx.x * HALF_HIDDEN_DIM + threadIdx.x;
    input2[idx] = __hadd2(input1[idx], input2[idx]);
}
...
void launchResidualConnectionKernelV2(
    ...
    residualConnectionKernelV2<<<token_num, HALF_HIDDEN_DIM>>>(
        reinterpret_cast<const __nv_bfloat162*>(input1),
        reinterpret_cast<__nv_bfloat162*>(input2)
    );
}
...
void residualConnectionKernelV3(
    ...
    for (int i = 0; i < bf162_per_thread; i++) {
        input2[idx + i] = __hadd2(input1[idx + i], input2[idx + i]);
    }
}
...
void launchResidualConnectionKernelV3(
    ...
    residualConnectionKernelV3<<<token_num, 256>>>(
        reinterpret_cast<const __nv_bfloat162*>(input1),
        reinterpret_cast<__nv_bfloat162*>(input2)
    );
}